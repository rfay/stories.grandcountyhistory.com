...
class AdminSettingsForm extends VersionedClass {
  ...
  public function build(array $form, FormState $formState) {
    ...
    $curl = !!$formState->getValue('curl.enabled', $this->config->get('curl.enabled', FALSE));
    $debug = !!$formState->getValue('debug.enabled', $this->config->get('debug.enabled', FALSE));
    $enabled = !!$formState->getValue('enabled', $this->config->get('enabled', '1'));
